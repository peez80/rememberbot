...
- **agy Parameter:** Es werden Standardparameter (`--prompt`, `--output-format stream-json`, `--dangerously-skip-permissions` sowie optional `--conversation <conversation_id>` zur nativen Kontextfortführung) verwendet. Der Aufruf ist in der Klasse `AgyClient` gekapselt.
